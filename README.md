...
<head>
    ...
    <title>منصة الإنجاز الملكية 2026</title>
...
<body class="p-4 md:p-8">
    ...
    <div id="authSection" class="min-h-screen flex items-center justify-center">
        <div class="glass-card p-12 w-full max-w-md text-center">
            <div class="mb-8 text-5xl animate-bounce">🪐</div>
            <h1 class="text-4xl font-black mb-8 tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-indigo-300 via-purple-300 to-pink-300">قائد المهمة 2026</h1>
            <div class="space-y-4">
                <input type="text" id="username" placeholder="تعريف القائد" class="w-full p-5 rounded-2xl input-style text-center outline-none">
                <input type="password" id="password" placeholder="كود الوصول" class="w-full p-5 rounded-2xl input-style text-center outline-none">
                <button onclick="handleLogin()" class="btn-prime w-full py-5 rounded-2xl font-black text-xl transition-transform active:scale-95">تفعيل النظام ⚡</button>
                <p class="text-[10px] text-gray-500 mt-4 uppercase tracking-widest">اضغط Enter للدخول</p>
            </div>
        </div>
    </div>
    ...
    <div id="mainApp" class="max-w-4xl mx-auto hidden">
        <header class="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
            ...
            <div class="flex bg-black/40 p-1.5 rounded-[22px] border border-white/5 backdrop-blur-xl">
                <button id="tabWeekBtn" onclick="switchTab('week')" class="tab-btn px-8 py-3 rounded-[18px] text-sm font-bold transition-all duration-500 tab-active">أسبوعي</button>
                <button id="tabYearBtn" onclick="switchTab('year')" class="tab-btn px-8 py-3 rounded-[18px] text-sm font-bold transition-all duration-500 text-gray-500">سنوي</button>
            </div>
            <button onclick="logout()" class="text-red-400/60 hover:text-red-400 font-bold">خروج</button>
        </header>
        ...
        <div id="luxuryWrapper" class="luxury-transition">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center gap-10">
                    <div class="flex-1 w-full text-right">
                        <div class="flex justify-between mb-4">
                            <h3 id="panelTitle" class="text-xs font-black text-indigo-400 uppercase tracking-[5px]">خطة الأسبوع</h3>
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            <div class="text-center group">
                                <span class="block text-[10px] text-gray-500 font-bold uppercase">الكل</span>
                                <span id="statTotal" class="text-3xl font-black italic">0</span>
                            </div>
                            <div class="text-center group">
                                <span class="block text-[10px] text-green-500/50 font-bold uppercase">منجز</span>
                                <span id="statDone" class="text-3xl font-black text-green-400 italic">0</span>
                            </div>
                            <div class="text-center group">
                                <span class="block text-[10px] text-purple-500/50 font-bold uppercase">باقي</span>
            ...
            <div class="glass-card p-3 mb-10">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                    <input type="text" id="taskInput" placeholder="ما هي المهمة التالية؟" class="md:col-span-6 p-6 rounded-[24px] bg-transparent outline-none text-xl font-bold">
                    ...
                    <div id="weekOptions" class="md:col-span-4 flex gap-3 p-2">
                        <select id="taskDay" class="flex-1 bg-white/5 rounded-2xl px-4 outline-none text-xs font-black border border-white/5 appearance-none">
                            ...
                            <option value="الثلاثاء">الثلاثاء</option><option value="الأربعاء">الأربعاء</option><option value="الخميس">الخميس</option><option value="الجمعة">الجمعة</option>
                        </select>
                    ...
                    <div id="yearOptions" class="md:col-span-4 flex gap-3 p-2 hidden">
                        <select id="taskCategory" class="flex-1 bg-white/5 rounded-2xl px-4 outline-none text-xs font-black border border-white/5 appearance-none">
                            <option value="شخصي">👤 شخصي</option><option value="عمل">💼 عمل</option><option value="صحة">🍎 صحة</option><option value="تعلم">📚 تعلم</option>
                        </select>
                        <input type="date" id="yearDeadline" class="w-32 bg-white/5 rounded-2xl px-4 outline-none text-[10px] border border-white/5">
                    </div>

                    <button onclick="addItem()" class="md:col-span-2 btn-prime rounded-2xl font-black py-4">إضافة</button>
                </div>
            </div>

            <div id="itemList" class="space-y-6 pb-24"></div>
        </div>
    </div>

<script>
    // دالة تبديل الحالة الجديدة
    ...
    function handleLogin() {
        ...
        if(auth[name] && auth[name] !== pass) return alert("كود الوصول خاطئ");
        auth[name] = pass;
    ...
    function switchTab(tab) {
        ...
        document.getElementById('panelTitle').innerText = tab === 'week' ? 'خطة الأسبوع' : 'خطة السنة';
        render();
    }
    ...
    function addItem() {
        ...
        if(currentTab === 'week') {
            ...
            item.deadline = document.getElementById('yearDeadline').value || "غير محدد";
        }
    ...
    function createItemEl(t) {
        ...
        div.innerHTML = `
            ...
            <button onclick="deleteItem(${t.id})" class="text-gray-600 hover:text-red-400">✕</button>
        `;
        return div;
    }
